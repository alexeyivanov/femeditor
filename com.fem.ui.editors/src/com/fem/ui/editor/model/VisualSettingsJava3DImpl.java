...
*
*/
public class VisualSettingsJava3DImpl implements VisualSettings, Cloneable {
	 ...
	 * @param vs
	 */
//	public VisualSettingsJava3DImpl(VisualSettings2 vs) {
////		this(vs.getFaceColor(), vs.getLineColor(), vs.getLineWidth(), vs.getTexture(), vs.getTransparncy());	
//	}
//	
	
	
	public <T> T getLineAppearance(final Class<T> clazz) {
	...
	public <T> T getSelectedLineAppearance(final Class<T> clazz) {
		Appearance a=new Appearance();
		a.setCapability(Appearance.ALLOW_COLORING_ATTRIBUTES_WRITE);
		a.setCapability(Appearance.ALLOW_LINE_ATTRIBUTES_WRITE);		
		ColoringAttributes ca=new ColoringAttributes();
	...
	public <T> T getMeshAppearance(final Class<T> clazz) {
		Appearance a=new Appearance();
	...
	public <T> T getSelectedMeshAppearance(final Class<T> clazz) {
		Appearance a=new Appearance();
		Material m1=new Material();
	...
	public <T> T getFaceAppearance(final Class<T> clazz) {
		Appearance a=new Appearance();
		Material m1=new Material();
		Color3f color=new Color3f(
				((float)faceColor.getRed())/255
	...
	public <T> T getSelectedFaceAppearance(final Class<T> clazz) {
		Appearance a=new Appearance();
		Material m1=new Material();
		Color faceColor = Color.RED;
		Color3f color=new Color3f(
	...
	public <T> T getSelectedTransparencyAttributes(final Class<T> clazz) {
		return (T) selectedTA;
	}
	
	public <T> T getScaleColor(final Class<T> clazz, float v) {		
		float R, G, B;
...
//		return (255 << 24) | (iR << 16) | (iG << 8) | iB;
	...
	public <T> T getJ3DColor(final Class<T> clazz, Color c) {
		return (T) new Color3f(((float)c.getRed())/255,((float)c.getGreen())/255,((float)c.getBlue())/255);
	}
	...
	public <T> T getTexture(final Class<T> clazz) {
		return (T) texture;
	}

	public <T> void setTexture(T texture, final Class<T> clazz) {
	...
	@Override
	public VisualSettings cloneObject() {
		return new VisualSettingsJava3DImpl(getFaceColor(), getLineColor(), getLineWidth(), getTexture(Texture2D.class), getTransparncy());
	}
}